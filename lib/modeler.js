...
async function calculateStatistics(snapshot) {
  ...
  for (let key in emoStats) {
    ...
    for(let i = 0; i < Object.keys(emoStats[key].numbers).length; i++) {
      ...
      stats[key].bounds[i] = {'upper': stats[key].averages[i] + (std * config.stats.bound_multiplier), 'lower': stats[key].averages[i] - (std * config.stats.bound_multiplier)}
    }
  }
  return stats
}
...
async function createModel() {
  ...
  model.add(tf.layers.dense({units: 32, activation: 'relu', inputShape: [config.tensorflow.input_size]}))
...
async function runTraining(batches) {
  ...
  await model.save('file://model/' + config.tensorflow.model_name + new Date().getTime() + '.json')
...
async function predict() {
  ...
  let values = predictionAnger.dataSync()
  let valuesObject = {};
  for(let i = 0; i <config.firestore.sort_order.length; i++) {
  ...
  valuesObject['title'] = 'Anger'
  console.log(valuesObject)
  snapshot.takesnap(valuesObject)
}
